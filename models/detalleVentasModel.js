...
class DetalleVentas {
  ...
  static async findById(folio, idProducto) {
    const result = await pool.query('SELECT * FROM DetalleVentas WHERE folio = $1 AND idProducto = $2', [folio, idProducto]);
  ...
  static async delete(folio, idProducto) {
    const result = await pool.query('DELETE FROM DetalleVentas WHERE folio = $1 AND idProducto = $2 RETURNING *', [folio, idProducto]);
    return result.rows[0];
  }
}
