...
class Proveedores {
  ...
  static async findById(id) {
    const result = await pool.query('SELECT * FROM Proveedores WHERE idProveedor = $1', [id]);
  ...
  static async delete(id) {
    const result = await pool.query('DELETE FROM Proveedores WHERE idProveedor = $1 RETURNING *', [id]);
    return result.rows[0];
  }
}
