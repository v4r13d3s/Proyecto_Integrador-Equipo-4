...
class MetodoVenta {
  static async findAll() {
    const result = await pool.query('SELECT * FROM MetodoVenta');
    return result.rows;  // Devuelve todos los métodos de venta
  }

  static async findById(id) {
    const result = await pool.query('SELECT * FROM MetodoVenta WHERE idMetodoVenta = $1', [id]);
  ...
  static async delete(id) {
    const result = await pool.query('DELETE FROM MetodoVenta WHERE idMetodoVenta = $1 RETURNING *', [id]);
